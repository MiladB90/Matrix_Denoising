...
def df_experiment(m: int, n: int, snr: float, snr2: float, p: float, noise_scale: float, soft_lvl: float,
                  max_matrix_dim: int, mc: int,
...
def do_matrix_denoising(*, m: int, n: int, snr: float, snr2: float, p: float, noise_scale: float, soft_lvl: float,
    ...
    return df_experiment(m=m, n=n, snr=snr, snr2=snr2, p=p, noise_scale=noise_scale, soft_lvl=soft_lvl,
                         max_matrix_dim=max_matrix_dim, mc=mc,
...
def test_experiment() -> dict:
    from experiment import make_tune_data
    ...
    sensing_model_table_name = 'milad_mc_0019'
    mc_range = (11, 11)

    make_tune_data(sensing_model_table_name)
    exp = dict(table_name='milad_md_test_make_tune_rank2',
               base_index=0,
